...
const CalendarRows = function PopulateCalendarRowsWithCalendarBoxes({ dateObject, onDateRender, onClick }) {
  const startOfMonth = moment(dateObject).startOf('month');
  const numberOfBlanksNeeded = startOfMonth.format('d');
  const days = [...getBlanks(numberOfBlanksNeeded), ...getDays(startOfMonth, onDateRender, onClick)];
  const rows = [];
  let cells = [];
